...
bgImage = PhotoImage(file = '/home/pi/Documents/slincam/Assets/Graphics/layoutvideo.png')
...
def framerate_clicked():
    if framerate_btn.config('text')[-1] == '24 FPS':
        framerate_btn.config(text='50 FPS')
        camera.framerate = 50
        
        
    else:
        framerate_btn.config(text='24 FPS')
        camera.framerate = 24
        
        

framerate_btn = Button(root, text="24 FPS", padx=7,pady=50,borderwidth=0,
                       highlightthickness = 0, fg="white", bg="#292929",
                       command=framerate_clicked)
framerate_btn['font'] = font1
framerate_btn.place(x=705, y=264)

framerate_btn.invoke()
...
menu_btn = Button(root, text="MENU", padx=23,pady=32,borderwidth=0,
                   highlightthickness = 0, fg="#292929", bg="#e2ca03",
...
stabl_btn = Button(root, text="~ OFF", padx=18,pady=41,borderwidth=0,
                   highlightthickness = 0, fg="white", bg="#292929",
                   command=stabl_btn_clicked)
stabl_btn['font'] = font1
stabl_btn.place(x=705, y=149)

#--------------------------------------------------------------------------------
                                                                            #REC.
def toggle_rec():
   
    if rec_button.config('text')[-1] == 'REC.':
        rec_button.config(text='STOP', fg="black",bg="red")
        camera.start_recording('/home/pi/Desktop/video.h264', format='h264',
                               profile='high', bitrate=50000000, level='4.2', sei='True')
        
        
    else:
        rec_button.config(text='REC.', fg="red",bg="#292929")
        camera.stop_recording()
        
        
    
                  
rec_button = Button(root,text="REC.",
                          padx=20,pady=56,borderwidth=0,highlightthickness = 0,fg="red",bg="#292929",
                          command=toggle_rec)
rec_button['font'] = font1
rec_button.place(x=705, y=6)
